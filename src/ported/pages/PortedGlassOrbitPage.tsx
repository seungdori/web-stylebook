        ...
        <main className="wrap">
          ...
          <section className="atmo-switcher" aria-label="Time of day palette">
            <div className="atmo-switcher__head">
              ...
              <span className="atmo-switcher__title">
                ...
                <span data-lang="ja" hidden>大気</span>
              </span>
          ...
          <section className="hero">
            ...
            <p className="lead" data-lang="ko" hidden>
              글래스모피즘 스타일을 단순 "투명 카드"로만 처리하면 밋밋해집니다. 이 페이지처럼 배경의 깊이,
              레이어 투명도 차이, 엣지 하이라이트를 같이 요청해야 밀도 있는 결과가 나옵니다.
            </p>
          ...
          <section className="specimen-section">
            ...
            <h2 data-lang="ja" hidden>標本 — 再生中</h2>
          ...
          <section className="refraction-section">
            ...
            <p className="refraction-lead" data-lang="ko" hidden>같은 구성, 다른 blur · saturate 조합. CSS 한 줄이 실제로 무엇을 바꾸는지 라이브로 보세요.</p>
            <p className="refraction-lead" data-lang="ja" hidden>同じ構成、異なるblur · saturateの組み合わせ。CSSの1行が何を変えるかライブで。</p>
    ...
    `}<span className="tok-v">inset</span>{` 0 0 60px `}<span className="tok-n">rgba</span>{`(100,180,255,0.12);
          ...
          <div className="desc-block">
            ...
            <p data-lang="ko" hidden>
              Glass Orbit는 세 가지 메커니즘으로 레이어드 투명도를 구축합니다: 서리 같은 깊이를 위한 <strong>backdrop-filter blur</strong>, 평면을 분리하는 <strong>점진적 border 불투명도</strong>, 내부 빛 굴절을 시뮬레이션하는 <strong>inset box-shadow</strong>. 각 요소는 서로 다른 깊이에 떠 있어, 평평한 캔버스에 그려진 것이 아니라 공간에 물리적으로 쌓여 있는 듯한 궤도 구성을 만듭니다.
            </p>
            <p data-lang="ja" hidden>
              Glass Orbitは3つのメカニズムでレイヤード透明度を構築します：フロストの深みを生む<strong>backdrop-filter blur</strong>、平面を分離する<strong>段階的なborder不透明度</strong>、内部の光屈折をシミュレートする<strong>inset box-shadow</strong>。各要素が異なる深さに浮かび、平らなキャンバスに描かれたのではなく、空間に物理的に積み重ねられた説得力のある軌道構成を作り出します。
            </p>
          </div>
