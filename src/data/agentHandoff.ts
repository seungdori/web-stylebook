...
export const verificationGroups: VerificationGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      t(
        'material claims, conclusions, and recommendations are traceable to the evidence or context that supports them; wording strength and certainty match that support, and missing or limited support is not hidden',
        '중요한 주장·결론·권고가 이를 뒷받침하는 근거나 맥락으로 추적 가능함; 문구의 강도와 확신 수준이 그 근거에 맞고, 근거가 없거나 제한적이라는 사실을 숨기지 않음',
        '重要な主張、結論、推奨は、それを支える根拠または文脈まで追跡できる。文言の強さと確信度はその裏付けに見合い、根拠がない、または限定的であることを隠さない',
      ),
      t(
        'prominent copy conveys concrete, context-specific information that helps the audience understand, decide, or act; it does not rely on interchangeable abstractions that remain unchanged when moved to an unrelated product or page',
        '주요 문구가 사용자가 이해·결정·행동하는 데 필요한 구체적이고 맥락에 맞는 정보를 전달함; 무관한 제품이나 페이지로 옮겨도 그대로 성립하는 교체 가능한 추상 표현에 의존하지 않음',
        '主要な文言は、利用者の理解、判断、行動に必要な具体的かつ文脈に即した情報を伝える。無関係な製品やページへ移してもそのまま成立する、置き換え可能な抽象表現に依存しない',
      ),
      t(
        'visual and structural prominence matches the content’s informational value, task relevance, and support; vague, generic, redundant, or weakly supported content does not become an oversized headline, card, badge, or statistic',
        '시각적·구조적 강조가 내용의 정보 가치·과업 관련성·근거 수준에 맞음; 모호하거나 범용적이거나 중복되거나 근거가 약한 내용을 거대한 제목·카드·뱃지·통계로 내세우지 않음',
        '視覚的・構造的な強調は、内容の情報価値、タスクとの関連性、裏付けの強さに見合う。曖昧、汎用的、重複、または根拠の弱い内容を巨大な見出し、カード、バッジ、統計として前面に出さない',
      ),
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      t('every interface design principle selected by get_design_principle_plan appears in design.md with the placement decision it produced and its observable verification check — not just its name', 'get_design_principle_plan으로 고른 인터페이스 디자인 원칙이 이름만이 아니라 그 원칙이 만든 배치 결정과 관찰 가능한 검증 기준까지 design.md에 남아 있음', 'get_design_principle_planで選んだインターフェースデザイン原則が、名前だけでなくその原則が生んだ配置の判断と観察可能な検証基準までdesign.mdに残っている'),
      ...
      t('each applicable check records pass, fail, not-run or not-applicable; pass includes the actual command or inspection and evidence, not-applicable includes a reason; remaining risk is recorded separately', '각 검증 항목에 통과·실패·미실행·해당 없음 상태를 기록하고, 통과에는 실제 명령 또는 확인 과정과 근거를, 해당 없음에는 이유를 적음. 남은 위험은 별도로 기록', '各検証項目に合格・不合格・未実行・非該当を記録し、合格には実際のコマンドまたは確認過程と証拠を、非該当には理由を添える。残るリスクは別に記録'),
...
export const antiPatterns: AntiPattern[] = [
  ...
  {
    id: 'unsupported-claim-as-established',
    pattern: t(
      'An unsupported claim or recommendation presented as established',
      '근거가 부족한 주장이나 권고를 확정된 내용처럼 제시',
      '根拠の乏しい主張や推奨を確定事項のように提示',
    ),
    why: t(
      'A statement becomes misleading when its wording carries more certainty or authority than the available evidence or context supports. Polished presentation can conceal that gap and make an assumption look established.',
      '문구가 사용 가능한 근거나 맥락보다 더 큰 확신과 권위를 띠면 오해를 만듭니다. 매끄러운 표현은 그 간극을 숨기고 가정을 확정된 내용처럼 보이게 할 수 있습니다.',
      '利用できる根拠や文脈を超える確信や権威を文言に持たせると、誤解を招きます。整った表現はその隔たりを隠し、仮定を確定事項のように見せることがあります。',
    ),
    fix: t(
      'For each material statement, identify its support and calibrate wording, certainty, and placement to that support. Show decision-relevant sources or limits; qualify or remove anything that cannot be supported, and do not invent evidence, facts, or next steps.',
      '중요한 문장마다 뒷받침하는 근거를 확인하고 문구의 강도·확신 수준·배치를 그 근거에 맞추세요. 판단에 필요한 출처나 한계를 드러내고, 뒷받침할 수 없는 내용은 한정해서 쓰거나 제거하며 근거·사실·다음 행동을 만들어내지 마세요.',
      '重要な文ごとに裏付けを確認し、文言の強さ、確信度、配置をその裏付けに合わせます。判断に必要な出典や限界を示し、支えられない内容は限定するか削除し、根拠、事実、次の行動を作り上げてはいけません。',
    ),
  },
  {
    ...
    why: t(
      'Abstract language can imitate expertise while leaving the actual information unspecified. If a sentence remains equally plausible after moving it to an unrelated product or page, it is likely adding tone rather than meaning.',
      '추상적인 표현은 실제 정보를 밝히지 않은 채 전문적인 분석처럼 보일 수 있습니다. 문장을 무관한 제품이나 페이지로 옮겨도 똑같이 그럴듯하다면 의미보다 분위기만 더하는 문구일 가능성이 큽니다.',
      '抽象的な表現は、実際の情報を示さないまま専門的な分析のように見せることがあります。無関係な製品やページへ移しても同じように成立する文は、意味より雰囲気を足している可能性が高いです。',
    ),
    fix: t(
      'State the concrete information the audience needs in plain language and tie it to the current page and task. If the sentence cannot become specific without inventing facts, remove it.',
      '사용자에게 필요한 구체적인 정보를 평범한 언어로 밝히고 현재 페이지와 과업에 연결하세요. 사실을 만들지 않고 구체화할 수 없다면 문장을 제거하세요.',
      '利用者に必要な具体的情報を平易な言葉で示し、現在のページとタスクに結び付けます。事実を作らずに具体化できない文は削除します。',
    ),
  },
  {
    ...
    why: t(
      'Large type, isolated cards, badges, and statistics assign importance to whatever they contain. Giving that treatment to vague, generic, redundant, or weakly supported content makes low-value content look like the page’s central result.',
      '큰 글자·고립된 카드·뱃지·통계는 그 안의 내용에 중요도를 부여합니다. 모호하거나 범용적이거나 중복되거나 근거가 약한 내용을 그렇게 강조하면 정보 가치가 낮은 내용이 페이지의 핵심 결과처럼 보입니다.',
      '大きな文字、独立したカード、バッジ、統計は、中身に重要性を与えます。曖昧、汎用的、重複、または根拠の弱い内容をそのように強調すると、情報価値の低い内容がページの中心的な結果のように見えます。',
    ),
    fix: t(
      'Replace the centerpiece with content whose information value, task relevance, and support earn that attention. Otherwise remove or demote it to supporting detail and reduce its visual scale.',
      '그 주목을 받을 만큼 정보 가치·과업 관련성·근거가 충분한 내용으로 중심을 바꾸세요. 그렇지 못하면 제거하거나 보조 상세로 내리고 시각적 크기를 줄이세요.',
      'その注目に見合う情報価値、タスクとの関連性、裏付けを持つ内容へ中心を置き換えます。できない場合は削除するか補助的な詳細へ下げ、視覚的な大きさを抑えます。',
    ),
  },
  {
    id: 'fake-content',
